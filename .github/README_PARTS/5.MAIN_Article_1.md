...
Откройте файл `DefaultEngine.ini`:

```
<Ваш_Проект>/Config/DefaultEngine.ini
```

- `Unreal Engine` автоматически собирает рабочий `Engine.ini` из фалов `DefaultEngine.ini` и других `Default*.ini` которые находятся в папке `Config`
